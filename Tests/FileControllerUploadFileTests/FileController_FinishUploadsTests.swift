...
class FileController_FinishUploadsTests: ServerTestCase, UploaderCommon {
    ...
    func runFinishUploadsWithNilFileGroup(test: FinishUploadsWithNilFileGroupTest) throws {
        ...
        guard let finishUploads = FinishUploadFiles(batchUUID: batchUUID, fileOwnerUserId: userId, sharingGroupUUID: sharingGroupUUID, objectType: nil, deviceUUID: deviceUUID, uploader: fakeUploader, params: params) else {
            XCTFail()
            return
        }
